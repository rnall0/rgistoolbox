...
get_ins<-function(file.name){
  ...
  #shp.df$source<-file_path_sans_ext(split[[1]][6])
  if(dir.exists(paste("geodata/clipped/", split[[1]][5], sep=""))){
    } else {
      dir.create(paste("geodata/clipped/", split[[1]][5], sep=""))
    }
  write.csv(shp.df, paste("geodata/clipped/", split[[1]][5], "/", file_path_sans_ext(split[[1]][6]), ".csv", sep=""), row.names = F, quote = TRUE)
  counter <<- counter + 1
